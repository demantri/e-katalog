												...
												?>
														<tr>
															...
															<td class="text-center">
																...
																<?php if($row['id_gaji'] != ''){ ?>
																		<a class='btn btn-primary btn-sm shadow' href="<?php echo site_url('transaksi/hr/gaji/daftar/detail/'.$row['id_gaji']) ?>" data-toggle="tooltip" title="Lihat Detail"><i class='fa fa-search'></i></a>
																<?php }else{ 
																		$d = cal_days_in_month(CAL_GREGORIAN,$row['bulan'],$row['tahun']);

				                    									if(strtotime(date('d')."-".date('m')."-".date('Y')) >= strtotime($d."-".$row['bulan']."-".$row['tahun'])){
				                    							?>
				                    										<a class="btn btn-primary btn-sm generateGaji" data-month="<?php echo $row['bulan'] ?>" 
				                    										   data-year="<?php echo $row['tahun'] ?>" href="javascript:void(0)">
				                    										   	<i class="fa fa-plus"></i> GENERATE GAJI
				                    										</a>
				                    							<?php
				                    									}else{
				                    							?>
				                    										<span class="badge badge-danger">Penggajian dapat dilakukan pada tanggal <?php echo $d ?></span>
				                    										<br><br>
				                    							<?php
				                    									}
				                    							?>
				                    										&nbsp;
				                    										<a data-toggle="tooltip" title="Preview" class="btn btn-warning btn-sm" href="<?php echo site_url('transaksi/hr/gaji/daftar/preview/'.$row['bulan'].'/'.$row['tahun']) ?>"><i class="fa fa-eye"></i></a>
				                    										<br><br>
				                    							<?php
																	  } 
																?>
